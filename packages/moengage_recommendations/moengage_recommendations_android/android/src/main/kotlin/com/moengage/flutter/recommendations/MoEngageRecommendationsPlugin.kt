...
class MoEngageRecommendationsPlugin : FlutterPlugin, ActivityAware {
    private val tag = "${MODULE_TAG}MoEngageRecommendationsPlugin"

    private val recommendationsHelper: RecommendationsHelper by lazy { RecommendationsHelper() }
    ...
    override fun onAttachedToEngine(binding: FlutterPluginBinding) {
        try {
            Logger.record {
                "$tag onAttachedToEngine() : Registering MoEngageRecommendationsPlugin"
            }
            ...
            Logger.record(PlatformLogLevel.ERROR, t) { "$tag onAttachedToEngine()  : " }
        }
    }
    ...
    override fun onDetachedFromEngine(binding: FlutterPluginBinding) {
        Logger.record { "$tag onDetachedFromEngine() : Detaching the Framework" }
    }
